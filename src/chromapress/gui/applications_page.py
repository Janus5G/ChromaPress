...
from PySide6.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QLabel, QPushButton, QTableWidget, QHeaderView, QFileDialog, QDialog, QDialogButtonBox, QLineEdit, QFormLayout, QGroupBox, QProgressBar, QTableWidgetItem, QMessageBox, QButtonGroup, QCheckBox, QTreeWidget, QTreeWidgetItem, QStyle
...
from chromapress.scenarios import SCENARIOS, relevance_score, application_section, suite_group, section_order_for_scenario, personal_home_group, HOME_GROUP_DESCRIPTIONS, missing_home_recommendations, HOME_EVERYDAY_SUBGROUPS, HOME_SPECIALIST_SUBGROUPS, HomeRecommendation, HOME_RECOMMENDATIONS, SCENARIO_RECOMMENDATIONS, home_recommendations_supported, scenario_recommendations_supported, missing_scenario_recommendations, SCENARIO_RECOMMENDATION_DESCRIPTIONS, scenario_by_id
...
class TextEntryDialog(QDialog):
    ...
    def __init__(self, title: str, field_label: str, help_text: str, placeholder: str='', parent=None):
        ...
        help_label = QLabel(tr(help_text))
        help_label.setWordWrap(True)
        layout.addWidget(help_label)
        box = QGroupBox(tr('Application source'))
        form = QFormLayout(box)
        self.value = QLineEdit()
        self.value.setPlaceholderText(tr(placeholder))
        self.value.setMinimumHeight(34)
        form.addRow(tr(field_label), self.value)
...
class GitEntryDialog(QDialog):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle(tr('Install/build from Git'))
        self.resize(760, 420)
        self.setMinimumSize(680, 360)
        layout = QVBoxLayout(self)
        heading = QLabel(tr('Install/build from Git'))
        heading.setObjectName('dialogTitle')
        layout.addWidget(heading)
        help_label = QLabel(tr('Enter the repository address. ChromaPress will inspect the source and build system in an isolated workspace before anything is staged for the image.'))
        help_label.setWordWrap(True)
        layout.addWidget(help_label)
        box = QGroupBox(tr('Git source'))
        form = QFormLayout(box)
        self.url = QLineEdit()
        self.url.setPlaceholderText(tr('https://github.com/owner/project.git'))
        self.ref = QLineEdit()
        self.ref.setPlaceholderText(tr('Optional branch, tag or commit'))
        self.url.setMinimumHeight(34)
        self.ref.setMinimumHeight(34)
        form.addRow(tr('Repository URL:'), self.url)
        form.addRow(tr('Version/ref:'), self.ref)
        layout.addWidget(box)
        layout.addStretch(1)
        buttons = QDialogButtonBox(QDialogButtonBox.StandardButton.Ok | QDialogButtonBox.StandardButton.Cancel)
        buttons.button(QDialogButtonBox.StandardButton.Ok).setText(tr('OK'))
...
class CatalogWorker(QObject):
    ...
    def run(self):
        try:
            ...
            result = bridge.application_catalog(str(self.source.get('path', '')), self.settings.workspace_dir, self.settings.reserve_gb, self.settings.cache_dir, str(self.source.get('sha256', '')))
...
class ApplicationsPage(QWidget):
    ...
    def __init__(self, settings, parent=None):
        ...
        layout = QVBoxLayout(self)
        title = QLabel(tr('Applications'))
        title.setObjectName('pageTitle')
        subtitle = QLabel(tr('Choose what programs you want in the Linux image. Start by selecting the intended use of the finished system; this only prioritises the application view and gives AI better context.'))
        subtitle.setWordWrap(True)
        layout.addWidget(title)
        layout.addWidget(subtitle)
        scenario_box = QGroupBox(tr('1. Intended system use'))
        scenario_layout = QGridLayout(scenario_box)
        scenario_layout.setHorizontalSpacing(8)
        scenario_layout.setVerticalSpacing(8)
        for column in range(3):
            scenario_layout.setColumnStretch(column, 1)
        self.scenario_group = QButtonGroup(self)
        self.scenario_group.setExclusive(True)
        self.scenario_buttons = {}
        for scenario_index, scenario in enumerate(SCENARIOS):
            button = QPushButton(tr(scenario.label))
            button.setCheckable(True)
            button.setToolTip(tr(scenario.ai_context))
            self.scenario_group.addButton(button)
            self.scenario_buttons[scenario.id] = button
            if scenario.id == 'custom':
                scenario_layout.addWidget(button, 2, 0, 1, 3)
            else:
        ...
        self.show_all = QCheckBox(tr('Show all applications'))
        self.show_all.setChecked(False)
        self.show_all.setEnabled(False)
        layout.addWidget(self.show_all)
        tools = QHBoxLayout()
        self.repo = QPushButton(tr('Add from repository'))
        self.url = QPushButton(tr('Direct link'))
        self.local = QPushButton(tr('Local package/file'))
        self.git = QPushButton(tr('Git repository'))
        self.ai = QPushButton(tr('Create with AI'))
        for b in (self.repo, self.url, self.local, self.git, self.ai):
            tools.addWidget(b)
        tools.addStretch(1)
        layout.addLayout(tools)
        status_row = QHBoxLayout()
        self.catalog_status = QLabel(tr('Open or download a Linux image to load its applications.'))
        self.catalog_status.setWordWrap(True)
        self.reload = QPushButton(tr('Reload applications'))
        self.reload.setEnabled(False)
        status_row.addWidget(self.catalog_status, 1)
        status_row.addWidget(self.reload)
        layout.addLayout(status_row)
        self.catalog_progress = QProgressBar()
        self.catalog_progress.setVisible(False)
        layout.addWidget(self.catalog_progress)
        self.search = QLineEdit()
        self.search.setPlaceholderText(tr('Search applications…'))
        self.search.setEnabled(False)
        layout.addWidget(self.search)
        self.tree = QTreeWidget()
        self.tree.setColumnCount(5)
        self.tree.setHeaderLabels([tr('Application'), tr('Version'), tr('Description'), tr('State'), tr('Action')])
        ...
        self.repo.clicked.connect(self.add_repo)
        self.url.clicked.connect(self.add_url)
        self.local.clicked.connect(self.add_local)
        self.git.clicked.connect(self.add_git)
        self.ai.clicked.connect(self.open_ai_requested)
        self.reload.clicked.connect(self.reload_catalog)
    ...
    def set_scenario(self, scenario_id: str, emit: bool=False) -> None:
        ...
        if selected:
            ...
            self.catalog_status.setText(tr('Choose the intended system use above to load and prioritise applications.'))
    ...
    def _cache_file(self) -> Path | None:
        ...
        if not cache_root or not key:
            return None
        safe = re.sub('[^a-zA-Z0-9._-]+', '-', key).strip('-') or 'unknown'
    ...
    def set_source(self, data: dict) -> None:
        ...
        if not data.get('path'):
            self.catalog_status.setText(tr('Open or download a Linux image to load its applications.'))
            return
        if self._try_load_windows_cache():
            return
        self.catalog_status.setText(tr('Preparing application list in the background — choose the intended system use above.'))
        self.reload_catalog()
    ...
    def reload_catalog(self) -> None:
        ...
        if not self.settings.workspace_dir.strip():
            self.catalog_status.setText(tr('BLOCKED — choose a Storage location in Settings first.'))
            return
        self.catalog_status.setText(tr('Reading user-facing applications from the selected image…'))
        ...
        self._thread.finished.connect(self._catalog_thread_finished)
    ...
    def _catalog_ready(self, result) -> None:
        ...
        if self._scenario_id:
            self.catalog_status.setText(tr(f'READY — {len(self._catalog)} installed applications loaded ({cache_note}).'))
            self.search.setEnabled(True)
            self._apply_filter(self.search.text())
        else:
            self.catalog_status.setText(tr(f'READY — {len(self._catalog)} applications loaded ({cache_note}). Choose the intended system use above.'))
            self.search.setEnabled(False)
            self.tree.clear()
        self.catalog_changed.emit(self.catalog_summary())
    ...
    def _catalog_failed(self, message: str) -> None:
        self.catalog_progress.setVisible(False)
        self.catalog_status.setText(tr('FAILED — application catalogue could not be read.'))
        QMessageBox.critical(self, tr('ChromaPress Applications'), tr(message))
    ...
    def set_staged_changes(self, changes: list[ChangeItem]) -> None:
        remove_packages = {str(change.payload.get('package', '')) for change in changes if change.kind == ChangeKind.PACKAGE_REMOVE and change.payload.get('package')}
        remove_refs = {self._change_target_ref(change) for change in changes if change.kind == ChangeKind.PACKAGE_REMOVE and self._change_target_ref(change)}
        replace_refs = {self._change_target_ref(change) for change in changes if change.kind == ChangeKind.PACKAGE_REPLACE and self._change_target_ref(change)}
        add_packages = {str(change.payload.get('package', '')) for change in changes if change.kind == ChangeKind.PACKAGE_REPOSITORY and change.payload.get('package')}
        if remove_packages == self._staged_remove_packages and remove_refs == self._staged_remove_refs and (replace_refs == self._staged_replace_refs) and (add_packages == self._staged_add_packages):
            return
    ...
    def catalog_summary(self) -> dict:
        ...
        return {'total': len(self._catalog) if self._catalog_loaded else None, 'read_method': self._catalog_read_method, 'scenario': self._scenario_id, 'sections': sections}
    ...
    def _leaf_item(self, parent: QTreeWidgetItem, app: dict) -> QTreeWidgetItem:
        ...
        keep = QPushButton(tr('Keep'))
        remove = QPushButton(tr('Remove'))
        replace = QPushButton(tr('Replace'))
        for button, width in ((keep, 66), (remove, 72), (replace, 78)):
            button.setCheckable(True)
            button.setMinimumWidth(width)
        choice = QButtonGroup(action_box)
        choice.setExclusive(True)
        choice.addButton(keep)
        choice.addButton(remove)
        choice.addButton(replace)
        action_box.setStyleSheet('QPushButton:checked { background: #dfe9f5; border: 1px solid #7f9fbe; font-weight: 600; }')
        if target_ref in self._staged_replace_refs:
            replace.setChecked(True)
            item.setText(3, tr('Replace staged'))
            keep.setToolTip(tr('Keep the installed application and undo the staged replacement.'))
            remove.setToolTip(tr('Change this staged replacement to a removal.'))
            replace.setToolTip(tr('Replacement is currently staged.'))
            ...
            item.setText(3, tr('Remove staged'))
            keep.setToolTip(tr('Keep the installed application and undo the staged removal.'))
            remove.setToolTip(tr('Removal is currently staged.'))
            replace.setToolTip(tr('Change this staged removal to a replacement.'))
            keep.clicked.connect(lambda _checked=False, ref=target_ref: self.undo_installed_change_requested.emit(ref))
            ...
            if package:
                keep.setToolTip(tr(f"Keep {app.get('application', package)} in the finished ISO."))
                remove.setToolTip(tr(f'Stage removal of package: {package}'))
            else:
                keep.setToolTip(tr('Keep this installed application unchanged.'))
                remove.setToolTip(tr('Stage removal using the installed launcher as an exact locator. Package ownership will be resolved fail-closed before apply.'))
            replace.setToolTip(tr('Choose a replacement package/application for this installed application.'))
            ...
            if not package and ownership == 'unresolved':
                item.setToolTip(3, tr('Installed launcher detected; package ownership will be resolved before Remove/Replace is applied.'))
        row.addWidget(keep)
    ...
    def _recommendation_item(self, parent: QTreeWidgetItem, recommendation: HomeRecommendation, *, context_scenario: str | None=None, label_prefix: str='Recommended') -> QTreeWidgetItem:
        item = QTreeWidgetItem(parent)
        item.setIcon(0, self._app_placeholder_icon)
        item.setSizeHint(0, QSize(0, 42))
        item.setText(0, tr(f'{label_prefix}: {recommendation.application}'))
        name_font = item.font(0)
        name_font.setBold(True)
        item.setFont(0, name_font)
        item.setText(1, tr(''))
        ...
        if package in self._staged_add_packages:
            add.setChecked(True)
            item.setText(3, tr(f'Add staged • {source_label}'))
            add.setToolTip(tr(f'{source_label} addition is currently staged.'))
            skip.setToolTip(tr(f'Skip {recommendation.application} and undo the staged addition.'))
            skip.clicked.connect(lambda _checked=False, pkg=package: self.undo_add_requested.emit(pkg))
        else:
            skip.setChecked(True)
            item.setText(3, tr(f'Available • {source_label}'))
            skip.setToolTip(tr('Leave the selected image unchanged for this recommendation.'))
            if manager == 'snap':
                add.setToolTip(tr(f'Stage Snap Store package: {package}'))
            elif manager == 'bundled':
                add.setToolTip(tr(f'Stage bundled application: {recommendation.application}'))
            else:
                add.setToolTip(tr(f'Stage repository package: {package}'))
            add.clicked.connect(lambda _checked=False, rec=recommendation, sid=context_scenario: self._stage_recommendation(rec, sid))
        row.addWidget(skip)
        row.addWidget(add)
        self.tree.setItemWidget(item, 4, action_box)
        ...
        if documentation_label and documentation_relpath:
            ...
            documentation.setText(2, tr('School evaluation material supplied with ChromaLearn; not a compliance certificate.'))
            documentation.setText(4, tr('Included automatically'))
    ...
    def _stage_recommendation(self, recommendation: HomeRecommendation, context_scenario: str | None=None) -> None:
        ...
        payload = {'package': recommendation.package, 'application': recommendation.application, 'recommendation_role': recommendation.role, 'recommended': True, 'manager': manager, 'scenario': recommendation_scenario}
        repository_component = str(getattr(recommendation, 'repository_component', '') or '').strip()
        ...
        self.change_requested.emit(ChangeItem(f'Add {recommendation.application}', ChangeKind.PACKAGE_REPOSITORY, detail, payload))

    @staticmethod
    def _recommendation_matches_query(recommendation: HomeRecommendation, query: str) -> bool:
        haystack = ' '.join((recommendation.application, recommendation.package, recommendation.description, recommendation.subgroup, ' '.join(recommendation.aliases))).casefold()
    ...
    def _global_search_recommendations(self, query: str) -> list[tuple[str, HomeRecommendation]]:
        ...
        installed_packages = {str(app.get('package', '')).strip() for app in self._catalog if str(app.get('package', '')).strip()}
    ...
    def _populate_global_search_recommendations(self, query: str) -> None:
        ...
        top.setText(0, tr(f'Available / recommended matches ({len(matches)})'))
        top.setText(2, tr('Global search results from supported application sources; independent of the selected system-use profile.'))
        font = top.font(0)
        font.setBold(True)
        top.setFont(0, font)
        for scenario_id, recommendation in matches:
            item = self._recommendation_item(top, recommendation, context_scenario=scenario_id, label_prefix='Available')
            scenario_label = scenario_by_id(scenario_id).label
            item.setToolTip(0, tr(f'Available recommendation from {scenario_label}.'))
    ...
    def _add_suite_or_leafs(self, parent: QTreeWidgetItem, apps: list[dict]) -> None:
        ...
        for suite_name, suite_apps in sorted(suites.items()):
            ...
            suite_item.setText(0, tr(f'{suite_name} ({len(suite_apps)})'))
            suite_font = suite_item.font(0)
            suite_font.setBold(True)
            suite_item.setFont(0, suite_font)
            if suite_name == 'LibreOffice':
                suite_item.setText(2, tr('Office suite — writing, spreadsheets, presentations, drawing and formulas. Expand to choose individual components.'))
            else:
                suite_item.setText(2, tr('Application suite — expand to view components.'))
            suite_item.setText(3, tr('Installed'))
    ...
    def _populate_personal(self, apps: list[dict]) -> None:
        ...
        if self._catalog_loaded:
            for recommendation in missing_home_recommendations(self._catalog, self._source, self._staged_remove_packages):
        ...
        subgroup_orders = {'Everyday essentials': HOME_EVERYDAY_SUBGROUPS, 'Specialised / Advanced': HOME_SPECIALIST_SUBGROUPS}
        for top in top_order:
            subgroups = grouped.get(top, {})
            subgroup_order = subgroup_orders[top]
            recommendation_count = sum((len(recommendations_by_subgroup.get(name, [])) for name in subgroup_order)) if top == 'Everyday essentials' else 0
            ...
            if recommendation_count:
                top_item.setText(0, tr(f'{top} ({total} installed, {recommendation_count} recommended)'))
            else:
                top_item.setText(0, tr(f'{top} ({total})'))
            ...
            for subgroup in subgroup_order:
                ...
                subgroup_recommendations = recommendations_by_subgroup.get(subgroup, []) if top == 'Everyday essentials' else []
                ...
                if subgroup_recommendations:
                    subgroup_item.setText(0, tr(f'{subgroup} ({len(subgroup_apps)} installed, {len(subgroup_recommendations)} recommended)'))
                else:
                    subgroup_item.setText(0, tr(f'{subgroup} ({len(subgroup_apps)})'))
                subgroup_item.setText(2, HOME_GROUP_DESCRIPTIONS.get(subgroup, 'Expand to view applications.'))
                subgroup_font = subgroup_item.font(0)
                subgroup_font.setBold(True)
                subgroup_item.setFont(0, subgroup_font)
    ...
    def _populate_scenario_recommendations(self) -> None:
        if not self._catalog_loaded or self._scenario_id in {'', 'personal', 'custom'} or self.show_all.isChecked() or self.search.text().strip():
            return
        recommendations = missing_scenario_recommendations(self._catalog, self._source, self._scenario_id, self._staged_remove_packages)
        ...
        top.setIcon(0, self.style().standardIcon(QStyle.StandardPixmap.SP_DirIcon))
        top.setSizeHint(0, QSize(0, 40))
        top.setText(0, tr(f'Recommended for {scenario.label} ({len(recommendations)})'))
        ...
        for subgroup, items in groups.items():
            ...
            subgroup_item.setIcon(0, self.style().standardIcon(QStyle.StandardPixmap.SP_DirOpenIcon))
            subgroup_item.setSizeHint(0, QSize(0, 38))
            subgroup_item.setText(0, tr(f'{subgroup} ({len(items)} recommended)'))
            subgroup_item.setText(2, tr('Recommended tools for this system role. Nothing is installed until Add is staged.'))
            subgroup_font = subgroup_item.font(0)
            subgroup_font.setBold(True)
            subgroup_item.setFont(0, subgroup_font)
    ...
    def _populate(self, apps: list[dict]) -> None:
        ...
        for section in sections:
            ...
            section_item.setText(0, tr(f'{section} ({len(section_apps)})'))
            section_descriptions = {'Internet & Communication': 'Browsers, communication and internet applications.', 'Office & Documents': 'Writing, documents, spreadsheets, presentations and readers.', 'Games': 'Games and entertainment applications.', 'Media & Graphics': 'Photos, graphics, audio, video and media applications.', 'Development': 'Programming, editors and software-development tools.', 'System & Utilities': 'System, file, settings and utility applications.', 'Other': 'Other installed user-facing applications.'}
    ...
    def _apply_filter(self, text: str) -> None:
        ...
        ranked = []
        for app in self._catalog:
            if query and (not (query in str(app.get('application', '')).casefold() or query in str(app.get('description', '')).casefold() or query in str(app.get('package', '')).casefold())):
                continue
            if query:
    ...
    def _installed_payload(self, app: dict) -> dict:
        ...
        return {'package': package, 'application': name, 'desktop_file': desktop_file, 'ownership': str(app.get('ownership', 'unresolved')), 'target_ref': self._app_ref(app)}
    ...
    def _stage_remove(self, app: dict) -> None:
        ...
        detail = f'Installed application • package {package}' if package else f"Installed application • resolve owner of {payload.get('desktop_file') or name} before apply"
        self.change_requested.emit(ChangeItem(f'Remove {name}', ChangeKind.PACKAGE_REMOVE, detail, payload))
    ...
    def _stage_replace(self, app: dict, undo_existing: bool=False) -> None:
        ...
        if not target_ref:
            return
        name = str(payload.get('application', 'Installed application'))
        dialog = TextEntryDialog(f'Replace {name}', 'Replacement application/package:', 'Enter the repository package that should replace this installed application. ChromaPress stages one atomic replacement plan: the current application is resolved and removed only if the replacement can be resolved safely.', 'Example: abiword', self)
        ...
        payload['manager'] = 'apt'
        self.change_requested.emit(ChangeItem(f'Replace {name} with {replacement}', ChangeKind.PACKAGE_REPLACE, f'Installed application → repository package {replacement}', payload))
    ...
    def add_url(self):
        ...
        if dialog.exec() and dialog.text():
            url = dialog.text()
            self.change_requested.emit(ChangeItem('Install from direct link', ChangeKind.DIRECT_URL, url, {'url': url}))

    def add_local(self):
        path, _ = QFileDialog.getOpenFileName(self, tr('Select Linux package or application'), '', tr('Linux packages/applications (*.deb *.rpm *.AppImage *.flatpak *.snap *.tar *.gz *.xz *.zst *.zip);;All files (*)'))
        if path:
            name = Path(path).name
            self.change_requested.emit(ChangeItem(f'Add {name}', ChangeKind.LOCAL_PACKAGE, path, {'path': path}))

    def add_git(self):
        dialog = GitEntryDialog(self)
        if dialog.exec() and dialog.url.text().strip():
            url = dialog.url.text().strip()
            ref = dialog.ref.text().strip()
            detail = url if not ref else f'{url} @ {ref}'
